...
void main() {
    ...
    // Background color: Black for Dark, White for Light
    vec3 bgColor = vec3(1.0 - isDark);
    ...
    if (t > 0.0) {
        ...
        // Blob color logic:
        // Dark Mode: Solid White (1.0) for max text contrast via mix-blend-exclusion
        // Light Mode: Black-ish center (fresnel) to stay visible on white background
        vec3 blobColor;
        if (isDark > 0.5) {
            blobColor = vec3(1.0); 
        } else {
            blobColor = vec3(fresnel); 
        }
...
function LavaLampShader({ theme }: { theme: 'light' | 'dark' }) {
...
export const LavaLamp = ({ theme = 'dark' }: { theme?: 'light' | 'dark' }) => {
  return (
    <div style={{ 
      width: '100%', 
      height: '100%', 
